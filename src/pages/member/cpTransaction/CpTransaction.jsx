...
import { useCpTransactionMember } from "../../../redux/features/member/cpTransaction/useCpTransactionMember";
...
const CpTransaction = () => {
  ...
  const { transactions, meta, isLoading, isFetching, error, changePage } =
    useCpTransactionMember(memberId);
  ...
  return (
    <div>
      <PageBreadcrumb
        items={[{ label: "Home", to: "/" }, { label: "CP Transaction" }]}
      />

      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white p-4">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          {/* <h1 className="text-xl font-semibold text-gray-900">
            My CP Transaction History
          </h1> */}
          {isFetching && (
                ...
                transactions.map((item, idx) => {
                  return (
                    <TableRow key={item?.id}>
                      ...
                      <TableCell className="text-center">
                        <Link
                          to={`/member/cp-transaction/${item?.id}`}
