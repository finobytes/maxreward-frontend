...
const SectionList = () => {
  ...
  const onSubmit = async (formData) => {
    try {
      ...
      if (editingSection) {
        ...
        res = await updateSection({ id: editingSection.id, data: payload }).unwrap();
      } else {
        // Create new section
      ...
      if (res?.success) {
        toast.success(res?.message || `Section ${editingSection ? 'updated' : 'created'} successfully!`);
        reset();
        setIsModalOpen(false);
        setEditingSection(null);
      } else {
        toast.error(res?.message || `Failed to ${editingSection ? 'update' : 'create'} section`);
      }
    } catch (err) {
  ...
  const handleEdit = (section) => {
    ...
    reset({
      name: section.name,
      status: section.status === true || section.status === "active" ? "active" : "inactive",
    });
    setIsModalOpen(true);
  };
