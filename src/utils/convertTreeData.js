export function convertToApexTreeFormat(apiData) {
  ...
  // Node builder
  const makeNode = (member, level) => ({
    id: member.id.toString(),
    data: {
      name: member.name,
      username: member.user_name || member.name,
      imageURL:
  ...
  treeLevels.forEach((level, idx) => {
    ...
    level.nodes.forEach((n) => {
      ...
      if (n.left_child) {
        const leftNode =
          nodeMap[n.left_child.id] || makeNode(n.left_child, currentLevel);
      ...
      if (n.right_child) {
        const rightNode =
          nodeMap[n.right_child.id] || makeNode(n.right_child, currentLevel);
